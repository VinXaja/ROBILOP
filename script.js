(function (global) {
    ...
    var
        ...
        function createMonthTable(data, option) {
            var table, tr, td, r, c, count, today = new Date(), todayDate = today.getDate(), todayMonth = today.getMonth(), todayYear = today.getFullYear();
            
            table = document.createElement("table");
            tr = document.createElement("tr");
            tr.setAttribute("class", "dycalendar-day-letters");
            ...
            for (c = 0; c <= 6; c++) {
                ...
                td.style.color = "black";  // Set the day letters color to black
                tr.appendChild(td);
            }
            ...
            while (c <= 6) {
                ...
                if (data.year < todayYear || (data.year === todayYear && data.monthIndex < todayMonth) || (data.year === todayYear && data.monthIndex === todayMonth && count < todayDate)) {
                    button.setAttribute("class", "dycalendar-disabled-date");
                    button.disabled = true;
                } else {
                    button.classList.add("active");
                    button.setAttribute("data-day", dayNames[c]);
                    button.setAttribute("data-date", count);
                    button.setAttribute("data-month", mmm[data.monthIndex]);
                    button.setAttribute("data-bs-dismiss",'modal');
                }
        
                // Highlight today's date
                if (data.today.date === count && data.today.monthIndex === data.monthIndex && option.highlighttoday === true) {
                    button.setAttribute("class", "dycalendar-today-date");
                    button.classList.add("active");
                    button.setAttribute("data-day", dayNames[c]);
                    button.setAttribute("data-date", count);
                    button.setAttribute("data-month", mmm[data.monthIndex]);
            ...
            for (r = 3; r <= 7; r++) {
                ...
                for (c = 0; c <= 6; c++) {
                    ...
                    if (data.today.date === count && data.today.monthIndex === data.monthIndex && option.highlighttoday === true) {
                        ...
                        button.setAttribute("data-bs-dismiss",'modal');
                        const container = document.getElementById('date');
                        container.innerHTML = `${dayNames[c]}, ${count} ${mmm[data.monthIndex]}`
                    }
    ...
    function drawCalendar(option) {
        ...
        activeBtn.forEach((btn) => {
            ...
            btn.addEventListener('click',() => date_input(day, date, month));
        })
    }
    ...
    function date_input (day,dat,mon) {
        const container = document.getElementById('date');
        container.innerHTML = `${day}, ${dat} ${mon}`;
    }
}
